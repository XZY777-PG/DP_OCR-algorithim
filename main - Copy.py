    ...
    while plc_connect_success:
        try:
            if plc.read_bool(siemens_plc, 1, 1) and not plc.read_bool(siemens_plc, 1, 2):
                ...
                # new_case_info.start()
            elif plc.read_bool(siemens_plc, 1, 1) == 0 and plc.read_bool(siemens_plc, 1, 2):
    ...
    while True:
        try:
            ...
            if len(df_ocr_sort_info[df_ocr_sort_info['ocr_finish_tm'].isna()]) > 0:
                ...
                if (datetime.datetime.now() - sensor_off_time).microseconds > 200:
