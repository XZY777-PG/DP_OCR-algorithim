...
if __name__ == '__main__':
    ...
    while True:
        now = datetime.datetime.now()
        print(plc.read_bool(ocs_siemens_plc, 1, 1),plc.read_bool(ocs_siemens_plc, 1, 2))
        if plc.read_bool(ocs_siemens_plc, 1, 1) and not plc.read_bool(ocs_siemens_plc, 1, 2):
    	    now = datetime.datetime.now()
            #image_capture_tm = now.strftime("%Y%m%d%H%M%S%f")            
    	    #print(image_capture_tm)
    	    #new_row = pd.Series({'serial_num': serial_num, 'image_capture_tm': image_capture_tm})
    	    ...
    	    # print(df_ocr_sort_info[df_ocr_sort_info['ocr_finish_tm'].isna()].loc[0]['image_capture_tm'])
    	    # new_case_info.start()
    	#elif not plc.read_bool(ocs_siemens_plc, 1, 1) and plc.read_bool(ocs_siemens_plc, 1, 2):
    	#else:
    	    #plc.write_bool(siemens_plc, 1, 2, 0)
    	    #program_run_once = True
    	    # print('stopped')
